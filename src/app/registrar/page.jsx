...
function page() {
  ...
  const getRazas = async () => {
    try {
      const razas = await axios.get("http://localhost:3000/api/races");
  ...
  const getCategory = async () => {
    try {
      const razas = await axios.get("http://localhost:3000/api/categories");
      const respuesta = razas.data.datos;
  ...
  const getGenders = async () => {
    try {
      const razas = await axios.get("http://localhost:3000/api/genders");
      const respuesta = razas.data.datos;
        ...
        >
          ...
          <div className="p-3 w-full bg-[#ffffffa5] outline-none flex justify-between rounded-[30px]">
            <input
              ...
              accept="image/*"
            />
            <span className="outline-none cursor-pointer">Subir Foto</span>
          ...
          <div>
            <button type="submit">
              <Image src={btnSave} alt="btn close" />
            </button>
          </div>
        </form>
      </div>
